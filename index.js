...
app.delete('/students/department', async (req, res) => {
    try {
        const{department}=req.query;
        if(!department){
            return res.status(400).send("Department is required to delete students.");
        }
        const result = await students.deleteMany({ department });
        res.status(200).send(`${result.deletedCount} document(s) deleted`);
    } catch (err) {
...
app.delete('/students/name/:name', async (req, res) => {
    try {
        const name = req.params.name; // Get the name from the route parameters

        if (!name) {
            return res.status(400).send("Name is required to delete a student.");
        }

        const result = await students.deleteOne({ name });
        if (result.deletedCount === 0) {
            return res.status(404).send(`No student found with name: ${name}`);
        }
